...
class CronTab(object):
    ...
    """
    def __init__(self, user=None, tab=None, tabfile=None, log=None, handler = None):
        self.lines = None
        self.crons = None
        self.filen = None
        self.env = None
    ...
    def read(self, filename=None):
        ...
        for line in lines:
            self.append(CronItem(line, cron=self,handler = self._handler), line, read=True)
    ...
    def run_timeout(self, **kwargs):
        """Run all commands in this crontab if timeout (sleeper)"""
        print "len(self):{} in run_pending \n".format(len(self))
        import sys
        sys.stdout.write("run_timeout\n")
        sys.stdout.flush()
        results = []
        for job in self:
            ret = job.run_pending(**kwargs)
    ...
    def get_sleeper(self, now = None, timeout = 1, **kwargs):
        sleeper = []
        for job in self:
            prev = job.get_prev_time()
            next = job.get_next_time()
            print "id:{} cronstring:{} ".format(job.id, job )
            print "compare last_run:{} prev:{} next:{}".format(job.last_run, prev, next)
            m = next
            sleeper.append(m)
        return sleeper
    
    def run_sleeper(self, now = None, timeout = 1, loop = 3, test = False, **kwargs):
        now = datetime.now()
        ...
        for count in range(loop):
            ...
            # sleep !
            if test == True:
                # pretend time has been passed
                now+= stime
            else:
                sec = stime.total_seconds()
                if sec < 1: # error handling
                    sec = 1
                sleep( sec )
                print "wake up!"
                now = datetime.now()
            self.run_timeout(now = now)
            # wait 3 seconds
            sleep(timeout)
    ...
    def new(self, line = None, command='', comment='', user=None, id=-1):
        """
        ...
        return self.append(CronItem(line, command, comment, user, self, handler = self._handler, id = id), line = line, read = True)
...
class CronItem(object):
    ...
    """
    def __init__(self, line=None, command='', comment='', user=None, cron=None, handler=None, id = -1):
        self.cron = cron
        self.user = user
        self.valid = False
        self.enabled = True
    ...
    def render(self):
        ...
        if self.id >= 0:
            result = result + " id:{} ".format(self.id)
        if not self.enabled:
            result = u"# " + result
    ...
    def run_pending(self, now=None):
        ...
        print "item.rn_pending is inovked\n"
        now = now or datetime.now()
        if self.is_enabled():
            #if self.last_run is None:
            #    self.last_run = now # bug !!! should not set the last_run to any value if it is None
            if self.last_run is not None:
                next_time = self.schedule(self.last_run).get_next()
                if next_time < now:
                    self.last_run = now
                    print "run ! it is timeout"
                    return self.run()
                else:
    ...
    def run(self):
        ...
        import sys
        print "start run of command:'{}'".format(self.command.encode('utf8'))
    ...
    def get_next_time(self):
        """get the next runtime since .last_run"""
        next = self.schedule(self.last_run).get_next()
        prev = self.schedule(self.last_run).get_prev()
        if self.last_run == None:
            return prev
        else:
            if self.last_run > prev:
                return next
            else:
                return prev
    ...
    def get_prev_time(self):
        """get the next runtime since .last_run"""
        return self.schedule(self.last_run).get_prev()        
    def schedule(self, date_from=None):
        """Return a croniter schedule if available."""
        #print "inside schedule of cronitem"
        if not date_from:
